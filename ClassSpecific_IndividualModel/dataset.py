...
class FGSBIR_Dataset(data.Dataset):
    def __init__(self, hp, mode):
        ...
        all_class.sort()
        current_class = np.array(all_class)[hp.class_id]
        hp.current_class = current_class
        train_sketch = [x for x in train_sketch if current_class == x.split('/')[0]]
        test_sketch = [x for x in test_sketch if current_class == x.split('/')[0]]
